...
export function NoteInputForm({ onAdd }: NoteInputFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 rounded-[24px] border border-white/10 bg-black/70 p-6 shadow-[0_0_0_1px_rgba(255,255,255,0.03)]">
      <div>
        <p className="text-sm uppercase tracking-[0.35em] text-zinc-500">New note</p>
        <h3 className="mt-2 text-2xl font-semibold text-white">Capture a thought you want to remember</h3>
      </div>
      <input
        value={title}
        onChange={(event) => setTitle(event.target.value)}
        placeholder="A small lesson"
      ...
      <textarea
        ...
        rows={5}
        placeholder="Write what matters while it is fresh"
        className="w-full rounded-3xl border border-white/10 bg-white/[0.04] px-4 py-3 text-sm text-white placeholder:text-zinc-500 focus:border-white/40 focus:outline-none focus:ring-2 focus:ring-white/10"
      />
